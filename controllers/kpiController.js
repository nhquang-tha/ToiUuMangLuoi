...
exports.getOptimizingData = async (req, res) => {
    ...
    try {
        ...
        // BƯỚC 3 & 4: TRUY VẾT DỮ LIỆU KPI 7 NGÀY & DỮ LIỆU CEM/P1
        ...
        targetCells.forEach(cellKey => {
            ...
            if (count > 0) {
                ...
                rows.forEach((r) => {
                    ...
                    sumThput += thputMbps; sumPrb += prb; sumCqi += cqi; sumDrop += drop; sumErab += erab; sumLatency += latency;
                ...
                avgThput = (sumThput / count).toFixed(2); 
            ...
            if ((thputCem > 0 && thputCem < 15) || videoBuf > 15 || initBuf > 10) cemIssues.push(`UXI 3 (Video): Buffer >15% hoặc Init >10s`);
            else if (parseFloat(avgThput) < 15) cemIssues.push(`UXI 3 (Video): Tốc độ < 15Mbps`);

            if (packetLoss > 1.6 || ulLatencyCem > 100) cemIssues.push(`UXI 2 (Data): Packet Loss > 1.6% hoặc Độ trễ > 100ms`);
            if (chatSuccess > 0 && chatSuccess < 95) cemIssues.push(`UXI 1 (Chat): Gửi tin thành công < 95%`);
            ...
            if (cellKey.startsWith('MBF_TH') || cellKey.startsWith('VNP_4G') || cellKey.startsWith('VNP-4G')) tier5Tags.push('RAN Sharing');
            if (p1Set.has(cellKey)) tier5Tags.push('P1 Tải Cao');
            else if (parseFloat(avgPrb) >= 75) tier5Tags.push('P1 Tải Cao');
            ...
            const item = {
                ...
                cemIssues: finalCemIssue, qosIssues: finalQosIssue, tier5Tags: tier5Tags,
