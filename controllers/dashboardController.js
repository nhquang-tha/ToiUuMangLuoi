...
// Hàm bổ trợ: Biến đổi thông minh cho số thập phân (Hỗ trợ cả 99,5 và 1,000.5)
const getSafeFloat = (val) => {
    if (val === undefined || val === null || val === '') return 0;
    let s = String(val).trim();
    if (s.includes(',') && !s.includes('.')) {
        s = s.replace(/,/g, '.'); // Đổi 99,5 thành 99.5
    } else if (s.includes(',') && s.includes('.')) {
        s = s.replace(/,/g, ''); // Đổi 1,000.5 thành 1000.5
    }
...
exports.handleImportData = async (req, res) => {
    ...
    for (const file of req.files) {
        try {
            ...
            // Xử lý CSV nếu file dùng dấu chấm phẩy làm vách ngăn
            ...
            // [TỐI ƯU TUYỆT ĐỐI]: TÌM DÒNG HEADER TRƯỚC TIÊN ĐỂ BAO THẦU CẢ CSV VÀ EXCEL
            if (networkType === 'mbb_qoe' || networkType === 'mbb_qos') {
                let bestHeaderIdx = -1;
                let maxMatch = 0;
            ...
            dataStartIdx = headerRowIdx + 1; // Mọi dòng bên dưới Header sẽ được quét tìm dữ liệu

            let excelHeaders = [];
            let h3_base = [];

            // Thuật toán Fill-forward (Rã đông Merged Cells) gom 3 dòng tiêu đề lại
            ...
            for (let c = 0; c < maxCols; c++) {
                let v1 = (h1 && h1[c] !== undefined && h1[c] !== null && String(h1[c]).trim() !== '') ? String(h1[c]).trim() : lastP1;
                let v2 = (h2 && h2[c] !== undefined && h2[c] !== null && String(h2[c]).trim() !== '') ? String(h2[c]).trim() : lastP2;
